...
enum ConnectionChangedReason {
  /// The SDK is connecting to Meta’s edge server.
  @JsonValue(0)
  Connecting,
  ...
  /// The connection between the SDK and Meta’s edge server is interrupted.
  @JsonValue(2)
  Interrupted,

  /// The connection between the SDK and Meta’s edge server is banned by Meta’s edge server.
  @JsonValue(3)
  BannedByServer,
  ...
  /// Timeout for the keep-alive of the connection between the SDK and Meta’s edge server. The connection state changes to:
...
enum ConnectionStateType {
  /// The SDK is disconnected from Meta's edge server.
  ...
  /// The SDK is connected to Meta's edge server and joins a channel. You can now publish or subscribe to a media stream in the channel.
  ...
  /// - If the SDK cannot rejoin the channel within 10 seconds after being disconnected from Meta’s edge server, the SDK triggers the [RtcEngineEventHandler.connectionLost] callback, stays in the [ConnectionStateType.Reconnecting] state, and keeps rejoining the channel.
  /// - If the SDK fails to rejoin the channel 20 minutes after being disconnected from Meta’s edge server, the SDK triggers the [RtcEngineEventHandler.connectionStateChanged] callback, switches to the [ConnectionStateType.Failed] state, and stops rejoining the channel.
  @JsonValue(4)
  Reconnecting,

  /// The SDK fails to connect to Meta's edge server or join the channel.
  /// You must call [RtcEngine.leaveChannel] to leave this state, and call [RtcEngine.joinChannel] again to rejoin the channel.
  ///
  /// If the SDK is banned from joining the channel by Meta’s edge server (through the RESTful API), the SDK triggers the [RtcEngineEventHandler.connectionStateChanged] callbacks.
  @JsonValue(5)
  Failed,
}
...
enum ErrorCode {
  ...
  /// The token expired. Meta recommends that you use [ConnectionChangedReason.TokenExpired] in the reason parameter of [RtcEngineEventHandler.connectionStateChanged] instead.
  ///
  /// Possible reasons are:
  /// - Authorized Timestamp expired: The timestamp is represented by the number of seconds elapsed since 1/1/1970. The user can use the token to access the Meta service within five minutes after the token is generated. If the user does not access the Meta service after five minutes, this token is no longer valid.
  /// - Call Expiration Timestamp expired: The timestamp is the exact time when a user can no longer use the Meta service (for example, when a user is forced to leave an ongoing call). When a value is set for the Call Expiration Timestamp, it does not mean that the token will expire, but that the user will be banned from the channel.
  @deprecated
  @JsonValue(109)
  TokenExpired,

  /// The token is invalid. Meta recommends that you use [ConnectionChangedReason.InvalidToken] in the reason parameter of [RtcEngineEventHandler.connectionStateChanged] instead.
  ///
  /// Possible reasons are:
  ...
  /// The Internet connection is interrupted. This applies to the Meta Web SDK only.
  @JsonValue(111)
  ConnectionInterrupted,

  /// The Internet connection is lost. This applies to the Meta Web SDK only.
  @JsonValue(112)
  ConnectionLost,
  ...
  /// An error occurs in Meta’s streaming server. Call the `addPublishStreamUrl` method to publish the stream again.
  ///
  ...
  /// Fails to start the camera. Meta recommends that you use [LocalVideoStreamError.CaptureFailure] in the error parameter of [RtcEngineEventHandler.localVideoStateChanged] instead.
  @deprecated
  @JsonValue(1003)
  StartCamera,
...
enum RtmpStreamingState {
  ...
  /// The SDK is connecting to Meta’s streaming server and the RTMP server. This state is triggered after you call the [RtcEngine.addPublishStreamUrl] method.
  @JsonValue(1)
  Connecting,
...
enum WarningCode {
  ...
  /// The server rejects the request to look up the channel. The server cannot process this request or the request is illegal. Meta recommends that you use [ConnectionChangedReason.RejectedByServer] in the reason parameter of [RtcEngineEventHandler.connectionStateChanged] instead.
  @deprecated
  @JsonValue(105)
  ...
  /// Audio device module: The audio recording device is different from the audio playback device, which may cause echoes problem. Meta recommends using the same audio device to record and playback audio.
  ///
  /// @since v3.1.2.
  @JsonValue(1042)
