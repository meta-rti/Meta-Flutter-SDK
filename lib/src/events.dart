...
class RtcEngineEventHandler {
  ...
  /// - Drop offline: When no data packet of the user or host is received for a certain period of time (20 seconds for the [ChannelProfile.Communication] profile, and more for the [ChannelProfile.LiveBroadcasting] profile), the SDK assumes that the user/host drops offline. A poor network connection may lead to false detections, so we recommend using the Meta RTM SDK for reliable offline detection.
  ///
  ...
  ///
  /// The Meta SDK returns this callback to report on the current network connection state when it changes, and the reason to such change.
  ///
  ...
  /// - On Android, the distance value reported in this callback may be slightly different from the actual distance. Therefore, Meta does not recommend using it for accurate calculation.
  ///
  ...
  ///
  /// Last mile refers to the connection between the local device and Meta's edge server. This callback reports once every two seconds the last mile network conditions of each user in the channel. If a channel includes multiple users, then this callback will be triggered as many times.
  ///
  ...
  ///
  /// When you call the [RtcEngine.startAudioMixing] method and the state of audio mixing file changes, the Meta SDK triggers this callback.
...
class RtcChannelEventHandler {
  ...
  /// Occurs when the network connection state changes.
  ///
  /// The Meta SDK triggers this callback to report on the current network connection state when it changes, and the reason to such change.
  ///
  /// The `ConnectionStateCallback` typedef includes the following parameters:
  /// - [ConnectionStateType] `state`: The current network connection state.
  /// - [ConnectionChangedReason] `reason`: The reason causing the change of the connection state.
