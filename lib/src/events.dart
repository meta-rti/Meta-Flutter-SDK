...
class RtcEngineEventHandler {
  ...
  /// - Drop offline: When no data packet of the user or host is received for a certain period of time (20 seconds for the [ChannelProfile.Communication] profile, and more for the [ChannelProfile.LiveBroadcasting] profile), the SDK assumes that the user/host drops offline. A poor network connection may lead to false detections, so we recommend using the Wuji RTM SDK for reliable offline detection.
  ///
  ...
  ///
  /// The Wuji SDK returns this callback to report on the current network connection state when it changes, and the reason to such change.
  ///
  ...
  /// - On Android, the distance value reported in this callback may be slightly different from the actual distance. Therefore, WUji does not recommend using it for accurate calculation.
  ///
  ...
  /// Reports the last mile network quality of the local user once every two seconds before the user joins the channel. Last mile refers to the connection between the local device and Wuji's edge server. After the application calls the [RtcEngine.enableLastmileTest] method, this callback reports once every two seconds the uplink and downlink last mile network conditions of the local user before the user joins the channel.
  ///
  ...
  ///
  /// Last mile refers to the connection between the local device and Wuji's edge server. This callback reports once every two seconds the last mile network conditions of each user in the channel. If a channel includes multiple users, then this callback will be triggered as many times.
  ///
  ...
  ///
  /// When you call the [RtcEngine.startAudioMixing] method and the state of audio mixing file changes, the Wuji SDK triggers this callback.
...
class RtcChannelEventHandler {
  ...
  /// - Go offline: When no data packet of the user or broadcaster is received for a certain period of time (around 20 seconds), the SDK assumes that the user/broadcaster drops offline. A poor network connection may lead to false detections, so we recommend using the Wuji RTM SDK for reliable offline detection.
  ///
  /// The `UserOfflineCallback` typedef includes the following parameters:
  /// - [int] `uid`: ID of the user or host who leaves the channel or goes offline.
  /// - [UserOfflineReason] `reason`: Reason why the user goes offline.
  ...
  /// Occurs when the network connection state changes.
  ///
  /// The Wuji SDK triggers this callback to report on the current network connection state when it changes, and the reason to such change.
  ///
  /// The `ConnectionStateCallback` typedef includes the following parameters:
  /// - [ConnectionStateType] `state`: The current network connection state.
  /// - [ConnectionChangedReason] `reason`: The reason causing the change of the connection state.
